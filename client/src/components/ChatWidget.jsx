    ...
    try {
      ...
      if (resp?.jobId) {
        ...
        const result = await pollChatResult({
          ...
          idToken,
          onProgress: (progress) => {
            // Update the last processing message with real-time agent badges
            setMessages(prev => {
              const updated = [...prev];
              // Find the processing message (last message with processing: true)
              const processingIndex = updated.findIndex((m, idx) => 
                m.processing === true && idx >= updated.length - 2
              );
              if (processingIndex !== -1) {
                const agentCount = (progress.agents_invoked || []).length;
                const agentText = agentCount > 0 ? ` • ${agentCount} agent${agentCount !== 1 ? 's' : ''} invoked` : '';
                updated[processingIndex] = {
                  ...updated[processingIndex],
                  agents_invoked: progress.agents_invoked || [],
                  text: `AI agents analyzing... ${Math.floor(progress.elapsedSeconds)}s elapsed${agentText}`
                };
              }
              return updated;
            });
        ...
        setMessages(prev => {
          ...
          const processingIndex = updated.findIndex(m => m.processing === true);
          if (processingIndex !== -1) {
            updated[processingIndex] = {
              role: 'assistant',
